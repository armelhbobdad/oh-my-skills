...
- Write all compiled artifacts to the staging directory `_bmad-output/{skill-name}-skill/`. Do not write to `skills/` or `forge-data/` — step-07 promotes staged artifacts to their final versioned locations.
...
Create `_bmad-output/{skill-name}-skill/` (and `_bmad-output/{skill-name}-skill/references/`). All artifacts produced in sections 2–7 below are written here:
...
- All content must be both in context and in `_bmad-output/{skill-name}-skill/` — no final files in `skills/` or `forge-data/` yet
- Proceed directly to validation after assembly is complete

## CRITICAL STEP COMPLETION NOTE

ONLY WHEN all 6 content artifacts (SKILL.md, context-snippet.md, metadata.json, references/, provenance-map.json, evidence-report.md) are assembled in context AND written to the staging directory `_bmad-output/{skill-name}-skill/` will you proceed to load `{nextStepFile}` for spec validation. Note: extraction-rules.yaml is generated by step-07 from extraction data, not assembled here.
